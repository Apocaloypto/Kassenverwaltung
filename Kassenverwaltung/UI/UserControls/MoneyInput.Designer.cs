namespace Kassenverwaltung.UI.UserControls
{
   partial class MoneyInput
   {
      /// <summary> 
      /// Required designer variable.
      /// </summary>
      private System.ComponentModel.IContainer components = null;

      /// <summary> 
      /// Clean up any resources being used.
      /// </summary>
      /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
      protected override void Dispose(bool disposing)
      {
         if (disposing && (components != null))
         {
            components.Dispose();
         }
         base.Dispose(disposing);
      }

      #region Component Designer generated code

      /// <summary> 
      /// Required method for Designer support - do not modify 
      /// the contents of this method with the code editor.
      /// </summary>
      private void InitializeComponent()
      {
         numInput = new NumericUpDown();
         ((System.ComponentModel.ISupportInitialize)numInput).BeginInit();
         SuspendLayout();
         // 
         // numInput
         // 
         numInput.DecimalPlaces = 2;
         numInput.Dock = DockStyle.Fill;
         numInput.Location = new Point(0, 0);
         numInput.Name = "numInput";
         numInput.Size = new Size(219, 23);
         numInput.TabIndex = 0;
         numInput.TextAlign = HorizontalAlignment.Right;
         numInput.ThousandsSeparator = true;
         // 
         // MoneyInput
         // 
         AutoScaleDimensions = new SizeF(7F, 15F);
         AutoScaleMode = AutoScaleMode.Font;
         Controls.Add(numInput);
         Name = "MoneyInput";
         Size = new Size(219, 23);
         ((System.ComponentModel.ISupportInitialize)numInput).EndInit();
         ResumeLayout(false);
      }

      #endregion

      private NumericUpDown numInput;
   }
}
